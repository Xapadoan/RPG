#include "addArmorDataToTab.h"

static int	setArmorEntrySurface(TabEntry *entry, Armor *armor) {
	...
	if (!armor | !entry) {
		fputs("Arg Error : Missing arguments for setArmorEntrySurface\n", stderr);
		return (0);
	}
	...
	entry_content[0] = (char *)calloc(1, (MY_str_len(armor->name) + 5) * sizeof(char));
	if (!entry_content[0]) {
		fputs("Memory Error : Failed to allocate memory for first field of armor entry content\n", stderr);
		return (0);
	}
	MY_str_append(entry_content[0], armor->name);

	entry_content[1] = MY_dec2str(armor->weight);
	if (!entry_content[1]) {
		fputs("Error : Failed to convert armor weigth to string\n", stderr);
		return (0);
	}

	entry_content[2] = MY_dec2str(armor->value);
	if (!entry_content[2]) {
		fputs("Error : Failed to convert armor value to string\n", stderr);
		return (0);
	}
	...
	if (!putFont(font, entry_content[0], font_color, entry->surface, &font_pos)) {
		fputs("Error : Failed to put armor name on entry surface\n", stderr);
		return (0);
	}
	...
	if (!putFont(font, entry_content[1], font_color, entry->surface, &font_pos)) {
		fputs("Error : Failed to put armor value on entry surface\n", stderr);
		return (0);
	}
	...
	if (!putFont(font, entry_content[2], font_color, entry->surface, &font_pos)) {
		fputs("Error : Failed to put armor weight on entry surface\n", stderr);
		return (0);
	}
...
int	addArmorDataToTab(TabMenu *menu, Armor *armor)
{
	TabEntry *entry;
	Tab *tab;
	char *armor_category;
	...
	armor_category = NULL;
	if (!menu | !armor) {
		fputs("Arg Error : Missing arguments for addArmorDataToTab\n", stderr);
		return (0);
	}
	...
	tab = menu->tabs->first;
	while (tab && tab->id != ARMOR_TAB_ID)
		tab = tab->next;
	if (tab->id != ARMOR_TAB_ID) {
		fputs("Error : Could not fing armor tab\n", stderr);
		return (0);
	}
	...
	//Set Surface
	if (!setArmorEntrySurface(entry, armor)) {
		fputs("Error : Failed to set armor entry surface\n", stderr);
		return (0);
	}

	//Set misc content
	entry->misc_content = (char *)calloc(1, (MY_str_len(armor->name) + 3) * sizeof(char));
	...
	MY_str_append(entry->misc_content, armor_category);
	free(armor_category);
